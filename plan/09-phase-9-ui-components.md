...
- [x] Ctrl+Alt+Space → вікно відкривається, фокус на пошуковому полі
- [x] Ввести кілька символів → через ~100 мс список фільтрується, перший елемент виділяється
- [x] ArrowDown/ArrowUp — навігація; не прокручується за межі
- [x] Enter → toast «Скопійовано» → вікно ховається → буфер містить текст
- [x] Escape при непорожньому запиті — очищує (вікно **не** ховається)
- [x] Escape при порожньому — вікно ховається
- [x] Повторне відкриття: поле порожнє, список повний

### Зашифрований сніпет (Сценарій S2)
- [x] Зашифрований сніпет позначений 🔒
- [x] Enter → PasswordModal, фокус на полі пароля
- [x] Невірний пароль → «Невірний пароль», поле очищується, фокус повертається
- [x] Правильний пароль → toast, вікно ховається
- [x] Під час розшифрування: кнопки та поле disabled, «Розшифрування…»

### CRUD
- [x] Ctrl+N → CreateModal; Ctrl+Enter → toast, список оновлюється
- [x] Валідація: title < 3 → помилка під полем, фокус
- [x] Ctrl+E → EditModal; для зашифрованого — textarea замінена повідомленням
- [x] Delete → DeleteConfirmModal; фокус на «Скасувати»; Enter не видаляє; клік «Видалити» → toast

### Модальні вікна
- [x] Focus trap: Tab/Shift+Tab — фокус не виходить за межі
- [x] Клік по overlay → закриття
- [x] Після закриття: фокус повертається на попередній елемент

### Теми та локалізація
- [x] Ctrl+Shift+T → тема перемикається, `theme-light` на `<html>`
- [x] Змінити мову → весь UI змінюється без перезавантаження
