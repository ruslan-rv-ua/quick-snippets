...
- [x] `cargo check` у `src-tauri/` завершується без помилок (всі залежності резолвляться)
- [x] `npm install` завершується без помилок
- [x] `npm run tauri dev` запускає порожнє вікно Tauri з заголовком `QuickSnippets`
- [x] Вікно не менше 480×320 пікселів, не перевищує початковий розмір 680×520
- [x] `npm run test` завершується успішно (0 тестів, 0 помилок)
- [x] `npm run test:rust` завершується успішно
